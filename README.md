...
- **24小时滑动窗口**: 自动清理超过24小时无更新的用户数据
- **续跑机制**: 跨天自动继续未完成的一轮，确保数据完整
- **每日定时**: 每天 UTC+8 次日 03:50 自动运行一轮
...
- **自动**: 每天 UTC+8 次日 03:50 自动运行
- **运行时长**: 约 4-6 小时（取决于名单人数）
...
1. **每轮目标**: 处理全部名单用户（配置 112 人，实际产出会根据用户活跃度动态变化）
2. **处理方式**: 每 5 人一批，批次间等待 3 分钟，用户间等待 3 分钟
3. **续跑逻辑**: 
   - 如果某天未完成（如处理了 40 人）
   - 第二天会先处理剩余 14 人，再处理新一轮的前 40 人
   - 确保每天都产出完整数据（共 54 人）
4. **数据清理**: 超过 24 小时无新推文的用户自动从 feed 中移除
